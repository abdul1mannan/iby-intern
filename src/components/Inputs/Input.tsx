...
interface InputProps {
	label: string;
	id: Extract<keyof FormDefaults, string>; // Ensure 'id' is only string keys
	type?: string;
	required?: boolean;
	register: UseFormRegister<FormDefaults>;
	errors: FieldErrors<FormDefaults>; // Ensure errors match the fields in FormDefaults
	disabled?: boolean;
}
		...
		<div>
			...
			<div className="relative mt-2">
				<input
					...
					autoComplete={id}
					disabled={disabled}
					{...register(id, { required })} // Register uses keyof FormDefaults now
					className={clsx(
